...
class BasePlugin:
    ...
    def __init__(self):
        ...
        self.MajDomoDevice_timing_cnt = self.MajDomoDevice_timing_cumul = self.MajDomoDevice_timing_avrg = self.MajDomoDevice_timing_max = 0
        self.ReadCluster_timing_cnt = self.ReadCluster_timing_cumul = self.ReadCluster_timing_avrg = self.ReadCluster_timing_max = 0
        self.ZigateRead_timing_cnt= self.ZigateRead_timing_cumul = self.ZigateRead_timing_avrg = self.ZigateRead_timing_max = 0
    ...
    def onStart(self):
        ...
        if LoadDeviceList( self ) == 'Failed' :
            ...
            self.log.logging( 'Plugin', 'Error',"Please cross-check your log ... You must be on V3 of the DeviceList and all DeviceID in Domoticz converted to IEEE")
            return            
    ...
    def onHeartbeat(self):
        ...
        if self.pluginconf.pluginConf['internetAccess'] and \
            ...
            self.pluginParameters['available'] , self.pluginParameters['available-firmMajor'], self.pluginParameters['available-firmMinor'] = checkPluginVersion( self.pluginParameters['PluginBranch'], self.FirmwareMajorVersion )
