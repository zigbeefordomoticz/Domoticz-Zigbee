    ...
    def profiling( f_in ):
        def f_out(self, decoded_frame):
            ...
            if self.pluginconf.pluginConf['ZiGateReactTime']:
                ...
                if t_elapse > 1000:
                   self.logging_receive( 'Log', "thread_process_messages (F_out) spend more than 1s (%s ms) frame: %s" %( t_elapse, decoded_frame))
            else:
                result = f_in(self, decoded_frame)
            return result
        return f_out
    ...
    def profiling( f_in ):
        def f_out(self, message):
            ...
            if self.pluginconf.pluginConf['ZiGateReactTime']:
                ...
                if t_elapse > 1000:
                    self.logging_receive( 'Log', "thread_process_messages (F_out) spend more than 1s (%s ms) frame: %s" %( t_elapse, message))
            else:
                result = f_in(self, message)
            return result
        return f_out
