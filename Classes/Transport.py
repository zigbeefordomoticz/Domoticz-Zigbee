...
class ZigateTransport(object):
    ...
    def __init__(
        ...
        self._waitFor8000Queue = (
            []
        )  # list of command sent and waiting for status 0x8000
    ...
    def start_thread_watchdog(self):
        if self.WatchDogThread is None:
            ...
            self.WatchDogThread = Thread(
                name="ZiGateWatchDog",
                target=ZigateTransport.threads_watchdog,
                args=(self,),
            )
    ...
    def threads_watchdog(self):
        ...
        while self.running:
            ...
            if self.running and not check_thread_alive(self.ListeningThread):
                Domoticz.Error(
                    "Thread %s seems to be dead, restarting" % self.ListeningThread.name
                )
    ...
    def serial_listen_and_send(self):
        ...
        Domoticz.Status(
            "ZigateTransport: Serial Connection open: %s" % serialConnection
        )
            ...
            if nb > 0:
                ...
                while nb:
                    try:
                        ...
                        Domoticz.Error(
                            "serial_listen_and_send - error while reading %s" % (e)
                        )
                        data = None
                    if data:
                ...
                while self.messageQueue.qsize() > 0:
                    ...
                    try:
                        ...
                        Domoticz.Error(
                            "serial_listen_and_send - error while writing %s" % (e)
                        )
    ...
    def tcpip_listen_and_send(self):
        ...
        while self.running:
            readable, writable, exceptional = select.select(
                inputSocket, outputSocket, inputSocket
            )
            if readable:
    ...
    def set_connection(self):
        def open_serial(self):
            try:
                self._connection = serial.Serial(
                    self._serialPort, baudrate=BAUDS, timeout=0
                )

            except serial.SerialException as e:
                Domoticz.Error(
                    "Cannot open Zigate port %s error: %s" % (self._serialPort, e)
                )
                return
        ...
        def open_tcpip(self):
            try:
                self._connection = socket.create_connection(
                    (self._wifiAddress, self._wifiPort)
                )
                self._connection.setblocking(0)

            except socket.Exception as e:
                Domoticz.Error(
                    "Cannot open Zigate Wifi %s Port %s error: %s"
                    % (self._wifiAddress, self._serialPort, e)
                )
                return
            if self.ListeningMutex is None:
                self.ListeningMutex = Lock()
        ...
        if self._transp in ["USB", "DIN", "PI"]:
            if (
                self._serialPort.find("/dev/") != -1
                or self._serialPort.find("COM") != -1
            ):
                Domoticz.Status(
    ...
    def sendData(self, cmd, datas, ackIsDisabled=False, waitForResponseIn=False):
        ...
        self.loggingSend(
            "Debug2",
            "   -> waitForResponse: %s waitForResponseIn: %s"
            % (waitForResponse, waitForResponseIn),
        )
        ...
        self.loggingSend(
            "Debug2",
            "sendData - %s %s ackDisabled: %s FIFO: %s"
        ...
        for x in list(self.ListOfCommands.keys()):
            if x not in self.ListOfCommands:
                continue
            if (
                "Status" not in self.ListOfCommands[x]
                or "Cmd" not in self.ListOfCommands[x]
                or "Datas" not in self.ListOfCommands[x]
            ):
                continue
            if (
                self.ListOfCommands[x]["Status"] in ("", "TO-SEND", "QUEUED")
                and self.ListOfCommands[x]["Cmd"] == cmd
                and self.ListOfCommands[x]["Datas"] == datas
            ):
                self.loggingSend(
        ...
        if alreadyInQueue:
            for x in list(self.ListOfCommands.keys()):
                ...
                self.loggingSend(
                    "Debug",
                    "-- > Sending Queue: [%s] Cmd: %s Datas: %s Time: %s"
                    % (
                        x,
                        self.ListOfCommands[x]["Cmd"],
                        self.ListOfCommands[x]["Datas"],
                        self.ListOfCommands[x]["ReceiveTimeStamp"].strftime(
                            "%m/%d/%Y, %H:%M:%S"
                        ),
                    ),
                )
        ...
        if InternalSqn in self.ListOfCommands:
            # Unexpected !
            Domoticz.Error(
                "sendData - Existing Internal SQN: %s for %s versus new %s/%s"
        ...
        if (
            (not ackIsDisabled or waitForResponse)
            and hexCmd in CMD_WITH_RESPONSE
            and hexCmd in RESPONSE_SQN
        ):
            self.ListOfCommands[InternalSqn]["MessageResponse"] = CMD_WITH_RESPONSE[
                hexCmd
            ]
    ...
    def on_message(self, Data):
        ...
        while 1:  # Loop until we have 0x03
            ...
            for val in self._ReqRcv[0 : len(self._ReqRcv)]:
                ...
                if (
                    Zero1 != -1 and val == 3
                ):  # If we have already started a Frame and do we get a 0x03
                    Zero3 = idx + 1
            ...
            if (
                Zero3 == -1
            ):  # No 0x03 in the Buffer, let's break and wait to get more data
                return

            if Zero1 != 0:
                Domoticz.Error(
                    "onMessage : we have probably lost some datas, zero1 = "
                    + str(Zero1)
                )
            ...
            if ComputedLength != ReceveidLength:
                ...
                Domoticz.Error(
                    "onMessage : Frame size is bad, computed = "
                    + str(ComputedLength)
                    + " received = "
                    + str(ReceveidLength)
                )
            ...
            if ComputedChecksum != ReceivedChecksum:
                ...
                Domoticz.Error(
                    "onMessage : Frame CRC is bad, computed = "
                    + str(ComputedChecksum)
                    + " received = "
...
def initMatrix(self):
    ...
    for x in ZIGATE_COMMANDS:
        self.loggingSend(
            "Debug2",
            "Command: %04x Ack: %s Sequence: %s/%s"
            % (
                x,
                ZIGATE_COMMANDS[x]["Ack"],
                len(ZIGATE_COMMANDS[x]["Sequence"]),
                ZIGATE_COMMANDS[x]["Sequence"],
            ),
        )
...
def send_data_internal(self, InternalSqn):
    ...
    if InternalSqn not in self.ListOfCommands:
        # Unexpected
        Domoticz.Error(
            "send_data_internal - unexpected 1 %s not in ListOfCommands: %s"
            % (InternalSqn, str(self.ListOfCommands.keys()))
        )
        return
    self.loggingSend(
        "Debug2",
        "--- send_data_internal - %s FIFO: %s"
    ...
    if (
        self.pdm_lock_status()
        and self.ListOfCommands[InternalSqn]["Cmd"] not in PDM_COMMANDS
    ):
        # Only PDM related command can go , all others will be dropped.
        Domoticz.Log(
            "PDM not yet ready, FIFO command %s %s"
            % (
    ...
    if sendNow and self.zmode == "zigate31c":
        ...
        if not self.ListOfCommands[InternalSqn]["PDMCommand"] and (
            len(self._waitFor8000Queue) > 0 or len(self._waitForCmdResponseQueue) > 0
        ):
            sendNow = False

        self.loggingSend(
            "Debug",
            "--- send_data_internal - Command: %s  Q(0x8000): %s Q(Response): %s sendNow: %s"
            % (
                self.ListOfCommands[InternalSqn]["Cmd"],
                len(self._waitFor8000Queue),
                len(self._waitForCmdResponseQueue),
                sendNow,
            ),
        )
        ...
        if not self.ListOfCommands[InternalSqn]["PDMCommand"] and (
            len(self._waitFor8000Queue) > 0
            or len(self._waitForAckNack) > 0
            or len(self._waitForCmdResponseQueue) > 0
        ):
            sendNow = False

        self.loggingSend(
            "Debug",
            "--- send_data_internal - Command: %s  Q(0x8000): %s Q(Ack/Nack): %s Q(Response): %s sendNow: %s"
            % (
                self.ListOfCommands[InternalSqn]["Cmd"],
                len(self._waitFor8000Queue),
                len(self._waitForAckNack),
                len(self._waitForCmdResponseQueue),
                sendNow,
            ),
        )
    ...
    self.loggingSend(
        "Debug",
        "--- send_data_internal - sending now zmode: %s Ack: %s ResponseExpected: %s"
        % (
            self.zmode,
    ...
    if not self.ListOfCommands[InternalSqn]["PDMCommand"]:
        ...
        if (
            self.ListOfCommands[InternalSqn]["ExpectedAck"]
            and self.zmode == "zigateack"
        ):
            set_acknack_for_sending(self, InternalSqn)

        if (
            self.zmode in "zigate31c"
            and self.ListOfCommands[InternalSqn]["ResponseExpected"]
        ):
...
def printListOfCommands(self, comment, isqn):
    ...
    self.loggingSend(
        "Debug", "  - Cmd:              %s" % self.ListOfCommands[isqn]["Cmd"]
    )
    self.loggingSend(
        "Debug", "  - Datas:            %s" % self.ListOfCommands[isqn]["Datas"]
    )
    self.loggingSend(
        "Debug", "  - ReTransmit:       %s" % self.ListOfCommands[isqn]["ReTransmit"]
    )
    self.loggingSend(
        "Debug", "  - Status:           %s" % self.ListOfCommands[isqn]["Status"]
    )
    self.loggingSend(
        "Debug",
        "  - ReceiveTimeStamp: %s" % self.ListOfCommands[isqn]["ReceiveTimeStamp"],
    )
    self.loggingSend(
        "Debug", "  - SentTimeStamp:    %s" % self.ListOfCommands[isqn]["SentTimeStamp"]
    )
    self.loggingSend(
        "Debug", "  - PDMCommand:       %s" % self.ListOfCommands[isqn]["PDMCommand"]
    )
    self.loggingSend(
        "Debug",
        "  - ResponseExpected: %s" % self.ListOfCommands[isqn]["ResponseExpected"],
    )
    self.loggingSend(
        "Debug",
        "  - MessageResponse:  %s" % self.ListOfCommands[isqn]["MessageResponse"],
    )
    self.loggingSend(
        "Debug", "  - ExpectedAck:      %s" % self.ListOfCommands[isqn]["ExpectedAck"]
    )
    self.loggingSend(
        "Debug",
        "  - WaitForResponse:  %s" % self.ListOfCommands[isqn]["WaitForResponse"],
    )
...
def set_cmdresponse_for_sending(self, i_sqn):
    ...
    if int(self.ListOfCommands[i_sqn]["Cmd"], 16) not in CMD_NWK_2NDBytes:
        if (
            self.ListOfCommands[i_sqn]["Cmd"] == "004E"
            and self.ListOfCommands[i_sqn]["Datas"][0:4] == "0000"
        ):
            ...
            self.ListOfCommands[i_sqn]["Cmd"] == "0049"
            and self.ListOfCommands[i_sqn]["Datas"][0:4] == "FFFC"
        ):
            self.loggingSend(
        ...
        if (
            self.ListOfCommands[i_sqn]["Datas"][0:2] == "%02x" % ADDRESS_MODE["group"]
            and self.ListOfCommands[i_sqn]["Datas"][2:6] == "0000"
        ):
            ...
            and self.ListOfCommands[i_sqn]["Cmd"] == "0110"
        ):
            ...
            self.loggingSend(
                "Debug",
                "--- 31c do not block 0110 even if Ack expected %s"
...
def set_acknack_for_sending(self, i_sqn):
    ...
    if (
        not self.firmware_with_aps_sqn
        and int(self.ListOfCommands[i_sqn]["Cmd"], 16) in CMD_NOACK_ZDP
    ):
...
def ready_to_send_if_needed(self):
    ...
    if self.zmode == "zigate31c":
        readyToSend = (
            len(self.zigateSendQueue) != 0
            and len(self._waitFor8000Queue) == 0
            and len(self._waitForCmdResponseQueue) == 0
        )
        self.loggingSend(
            "Debug2",
            "--- send_data_internal - Q(0x8000): %s Q(Ack/Nack): %s sendNow: %s"
            % (
                len(self.zigateSendQueue),
                len(self._waitFor8000Queue),
                len(self._waitFor8000Queue),
            ),
        )
...
def _send_data(self, InternalSqn):
    ...
    self.loggingSend(
        "Debug",
        "======================== _send_data - [%s] %s %s ExpectAck: %s ExpectResponse: %s"
        % (
            InternalSqn,
            cmd,
            datas,
    ...
    if datas == "":
        ...
        lineinput = (
            "01"
            + str(zigate_encode(cmd))
            + str(zigate_encode(length))
...
def check_timed_out(self):
    def timeout_8000(self):
        ...
        if InternalSqn in self.ListOfCommands:
            if (
                ...
                and self.ListOfCommands[InternalSqn]["ExpectedAck"]
            ):
                _next_cmd_to_wait_for_ack_nack_queue(self)

            if (
                self.zmode == "zigateack"
                and self.ListOfCommands[InternalSqn]["WaitForResponse"]
            ):
    ...
    def check_and_timeout_listofcommand(self):
        ...
        self.loggingSend(
            "Debug",
            "-- checkTimedOutForTxQueues ListOfCommands size: %s"
            % len(self.ListOfCommands),
        )
        for x in list(self.ListOfCommands.keys()):
            if (
                "SentTimeStamp" in self.ListOfCommands[x]
                and self.ListOfCommands[x]["SentTimeStamp"]
                and (now - self.ListOfCommands[x]["SentTimeStamp"]) > TIME_OUT_LISTCMD
            ):
                if self.ListOfCommands[x]["MessageResponse"]:
                    self.loggingSend(
                        "Debug",
                        " --  --  --  > - Time Out : [%s] %s %s Flags: %s/%s %04x Status: %s Time: %s"
                        % (
                            x,
                            self.ListOfCommands[x]["Cmd"],
                            self.ListOfCommands[x]["Datas"],
                            self.ListOfCommands[x]["ResponseExpected"],
                            self.ListOfCommands[x]["ExpectedAck"],
                            self.ListOfCommands[x]["MessageResponse"],
                    ...
                    self.loggingSend(
                        "Debug",
                        " --  --  --  > - Time Out : [%s] %s %s Flags: %s/%s Status: %s Time: %s "
                        % (
                            x,
                            self.ListOfCommands[x]["Cmd"],
                            self.ListOfCommands[x]["Datas"],
    ...
    def logExpectedCommand(self, desc, now, TimeStamp, i_sqn):
        ...
        if self.ListOfCommands[i_sqn]["MessageResponse"]:
            self.loggingSend(
                "Log",
                " --  --  --  > Time Out %s [%s] %s sec for  %s %s %s/%s %04x Time: %s"
                % (
                    desc,
                    i_sqn,
                    (now - TimeStamp),
                    self.ListOfCommands[i_sqn]["Cmd"],
                    self.ListOfCommands[i_sqn]["Datas"],
                    self.ListOfCommands[i_sqn]["ResponseExpected"],
                    self.ListOfCommands[i_sqn]["ExpectedAck"],
                    self.ListOfCommands[i_sqn]["MessageResponse"],
                    self.ListOfCommands[InternalSqn]["ReceiveTimeStamp"].strftime(
                        "%m/%d/%Y, %H:%M:%S"
                    ),
                ),
            )
            ...
            self.loggingSend(
                "Log",
                " --  --  --  > Time Out %s [%s] %s sec for  %s %s %s/%s %s Time: %s"
                % (
                    desc,
                    i_sqn,
                    (now - TimeStamp),
        ...
        self.loggingSend(
            "Debug",
            "--  --  --  > i_sqn: %s App_Sqn: %s Aps_Sqn: %s Type_Sqn: %s"
            % (
                i_sqn,
                self.ListOfCommands[i_sqn]["APP_SQN"],
                self.ListOfCommands[i_sqn]["APS_SQN"],
                self.ListOfCommands[i_sqn]["TYP_SQN"],
            ),
        )
    ...
    self.loggingSend(
        "Debug2",
        "checkTimedOut  Start - Aps_Sqn: %s waitQ: %2s ackQ: %2s dataQ: %2s SendingFIFO: %3s"
        % (
            self.firmware_with_aps_sqn,
            len(self._waitFor8000Queue),
            len(self._waitForAckNack),
            len(self._waitForCmdResponseQueue),
            len(self.zigateSendQueue),
    ...
    self.logging_receive(
        "Debug2",
        "checkTimedOut  End   - waitQ: %2s ackQ: %2s dataQ: %2s SendingFIFO: %3s"
        % (
            len(self._waitFor8000Queue),
            len(self._waitForAckNack),
            len(self._waitForCmdResponseQueue),
...
def process_frame(self, frame):
    def cleanup_8000_queues(self, status, isqn):
        ...
        if (
            self.zmode == "zigateack"
            and not self.ListOfCommands[isqn]["ExpectedAck"]
    ...
    self.logging_receive(
        "Debug",
        "process_frame - MsgType: %s MsgLength: %s MsgCRC: %s"
        % (MsgType, MsgLength, MsgCRC),
    )
    ...
    if MsgType == "8702":
        ...
        self.logging_receive(
            "Debug",
            "process_frame - APS Failure MsgType: %s MsgLength: %s MsgCRC: %s"
            % (MsgType, MsgLength, MsgCRC),
        )
    ...
    if int(MsgType, 16) in STANDALONE_MESSAGE:
        self.logging_receive(
            "Debug",
            "process_frame - STANDALONE_MESSAGE MsgType: %s MsgLength: %s MsgCRC: %s"
            % (MsgType, MsgLength, MsgCRC),
        )
        self.F_out(frame, None)  # for processing
        ready_to_send_if_needed(self)
    ...
    if MsgData and MsgType == "8002":
        self.logging_receive(
            "Debug",
            "process_frame - 8002 MsgType: %s MsgLength: %s MsgCRC: %s"
            % (MsgType, MsgLength, MsgCRC),
        )
    ...
    if (
        len(self._waitFor8000Queue) == 0
        and len(self._waitForCmdResponseQueue) == 0
        and len(self._waitForAckNack) == 0
    ):
        self.F_out(frame, None)
        ready_to_send_if_needed(self)
        return
    ...
    if MsgData and MsgType == "8000":
        ...
        if self.zmode == "auto":
            self.zmode = "zigate31c"
            self.loggingSend("Status", "Firmware < 31d switching to Mode: ZiGate31c")
        ...
        self.loggingSend(
            "Debug",
            "0x8000 - [%s] sqn_app: 0x%s/%3s, SQN_APS:non 0x%s type_sqn: %s"
        ...
        if i_sqn in self.ListOfCommands:
            ...
            self.loggingSend(
                "Debug",
                "--> Check cleanup Status: %s [%s] Cmd: %s Data: %s ExpectedAck: %s ResponseExpected: %s"
                % (
                    Status,
                    i_sqn,
                    self.ListOfCommands[i_sqn]["Cmd"],
    ...
    if self.zmode in "zigate31c":
        ...
        if not self.firmware_with_aps_sqn and MsgType in ("8100", "8110", "8102"):
            ...
            self.loggingSend(
                "Debug",
                "--> zigbee31c Receive MsgType: %s with ExtSqn: %s"
                % (MsgType, MsgZclSqn),
            )
        ...
        self.loggingSend(
            "Debug",
            "--> zigbeeack Receive MsgType: %s with ExtSqn: %s" % (MsgType, MsgZclSqn),
        )
...
def process_msg_type8000(self, Status, PacketType, sqn_app, sqn_aps, type_sqn):
    ...
    self.loggingSend(
        "Debug",
        "--> process_msg_type8000 - Status: %s PacketType: %s sqn_app:%s sqn_aps: %s type_sqn: %s"
    ...
    self.loggingSend(
        "Debug",
        " --  --  -- - > InternSqn: %s ExternalSqn: %s ExternalSqnZCL: %s"
    ...
    if self.pluginconf.pluginConf["ZiGateReactTime"]:
        ...
        if self.statistics._averageTiming8000 != 0 and timing >= (
            3 * self.statistics._averageTiming8000
        ):
            Domoticz.Log(
                "Zigate round trip time seems long. %s ms for %s %s SendingQueue: %s LoC: %s"
                % (
                    timing,
                    self.ListOfCommands[InternalSqn]["Cmd"],
                    self.ListOfCommands[InternalSqn]["Datas"],
        ...
        if not IsCommandOk:
            self.loggingSend(
                ...
                % (
                    ...
                    PacketType,
                    len(self._waitFor8000Queue),
                    str(self._waitFor8000Queue),
                    str(self.ListOfCommands),
                ),
            )
            return None
    ...
    if (
        not self.firmware_with_aps_sqn
        and self.ListOfCommands[InternalSqn]["ExpectedAck"]
    ) or (self.firmware_with_aps_sqn and type_sqn):
...
def process_msg_type8011_below31c(self, Status, NwkId, Ep, MsgClusterId, ExternSqn):
    ...
    self.loggingSend(
        "Debug",
        "--> process_msg_type8011_below31c - Status: %s ExternalSqn: %s NwkId: %s Ep: %s ClusterId: %s"
    ...
    if self.firmware_with_aps_sqn:
        ...
        if InternSqn != InternSqn_from_ExternSqn:
            Domoticz.Error(
                "process_msg_type8011_below31c different sqn : InternSqn:%s InternSQN_from_ExternalSQN:%s"
    ...
    if Status == "00":
        if InternSqn in self.ListOfCommands:
            self.loggingSend(
                "Debug",
                " - [%s] receive Ack for Cmd: %s - size of SendQueue: %s"
                % (
                    InternSqn,
                    self.ListOfCommands[InternSqn]["Cmd"],
                    self.loadTransmit(),
                ),
            )
        self.statistics._APSAck += 1
        ...
        if InternSqn in self.ListOfCommands:
            self.loggingSend(
                "Debug",
                " - [%s] receive Nack for Cmd: %s - size of SendQueue: %s"
                % (
                    InternSqn,
                    self.ListOfCommands[InternSqn]["Cmd"],
                    self.loadTransmit(),
...
def process_msg_type8011_above31d(self, Status, NwkId, Ep, MsgClusterId, ExternSqn):
    ...
    if Status == "00":
        if InternSqn in self.ListOfCommands:
            self.loggingSend(
                "Debug",
                " - Above 3.1d [%s] receive Ack for Cmd: %s - size of SendQueue: %s"
                % (
                    InternSqn,
                    self.ListOfCommands[InternSqn]["Cmd"],
                    self.loadTransmit(),
        ...
        if InternSqn in self.ListOfCommands:
            self.loggingSend(
                "Debug",
                " - Above 3.1d [%s] receive Nack for Cmd: %s - size of SendQueue: %s"
                % (
                    InternSqn,
                    self.ListOfCommands[InternSqn]["Cmd"],
                    self.loadTransmit(),
...
def process_msg_type8702(self, MsgData):
    ...
    self.loggingSend(
        "Debug",
        "process_msg_type8702 - ExternalSqn: %s NwkId: %s Ep: %s"
    ...
    self.loggingSend(
        "Debug",
        "----------->  ExternalSqn: %s InternalSqn: %s" % (ExternSqn, InternSqn),
    )

    return InternSqn


def process_other_type_of_message31c(
    self, MsgType, MsgSqn=None, MsgNwkId=None, MsgEp=None, MsgClusterId=None
):
    ...
    if InternalSqn not in self.ListOfCommands:
        Domoticz.Error(
            "process_other_type_of_message - MsgType: %s, InternalSqn: %s not found in ListOfCommands: %s"
    ...
    if (expResponse == MsgType) or (
        expResponse == 0x8100 and MsgType in ("8100", "8102")
    ):
        ...
        self.loggingSend(
            "Debug",
            " --  -- - > Expecting: %s %s %s receiving %s %s %s"
        ...
        if MsgSqn is None:
            Domoticz.Error(
                "process_other_type_of_message - MsgType: %s cannot get i_sqn due to unknown External SQN"
                % (MsgType)
            )
            return None
        ...
        self.loggingSend(
            "Debug",
            " --  -- - > Expected IntSqn: %s Received ISqn: %s ESqn: %s"
        ...
        if isqn and InternalSqn != isqn:
            ...
            self.loggingSend(
                "Debug",
                " --  -- - > Expecting: %04x Receiving: %s" % (expResponse, MsgType),
            )
            self.loggingSend(
                "Debug",
                " --  -- - > Expected IntSqn: %s Received ISqn: %s ESqn: %s"
    ...
    self.loggingSend(
        "Debug",
        " --  -- - > Internal SQN: %s Received: %s and expecting %04x"
...
def process_other_type_of_message31d(self, MsgType, MsgSqn):
    ...
    if MsgSqn is None:
        Domoticz.Error(
            "process_other_type_of_message31d - MsgType: %s cannot get i_sqn due to unknown External SQN"
            % (MsgType)
        )
        return None
    ...
    if InternalSqn not in self.ListOfCommands:
        Domoticz.Error(
            "process_other_type_of_message31d - MsgType: %s, InternalSqn: %s not found in ListOfCommands: %s"
            % (MsgType, InternalSqn, str(self.ListOfCommands.keys()))
        )
        ready_to_send_if_needed(self)
        return None
    ...
    cmd = None
    for x in ZIGATE_COMMANDS:
        if len(ZIGATE_COMMANDS[x]["Sequence"]) == 2 and ZIGATE_COMMANDS[x]["Sequence"][
            1
        ] == int(MsgType, 16):
            cmd = x
            break
...
def process8002(self, frame):
    ...
    self.logging_receive(
        "Debug",
        "process8002 NwkId: %s Ep: %s Cluster: %s Payload: %s"
    ...
    (
        ...
        Command,
        Data,
    ) = retreive_cmd_payload_from_8002(Payload)
    ...
    self.logging_receive(
        "Debug",
        "process8002 Sqn: %s ManufCode: %s Command: %s Data: %s "
        % (Sqn, ManufacturerCode, Command, Data),
    )
    if Command == "00":  # Read Attribute
        return buildframe_read_attribute_request(
            frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, ManufacturerCode, Data
        )

    if Command == "01":  # Read Attribute response
        return buildframe_read_attribute_response(
            frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
        )

    if Command == "04":  # Write Attribute response
        return buildframe_write_attribute_response(
            frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
        )

    if Command == "07":
        return buildframe_configure_reporting_response(
            frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
        )

    if Command == "0a":
        return buildframe_report_attribute_response(
            frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
        )

    self.logging_receive(
        "Log",
        "process8002 Unknown Command: %s NwkId: %s Ep: %s Cluster: %s Payload: %s"
        % (Command, SrcNwkId, SrcEndPoint, ClusterId, Data),
    )
...
def extract_nwk_infos_from_8002(frame):
    ...
    if ProfileId != "0104":
        Domoticz.Log(
            "extract_nwk_infos_from_8002 - Not an HA Profile, let's drop the packet %s"
            % MsgData
        )
...
def buildframe_read_attribute_request(
    frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, ManufacturerCode, Data
):
    ...
    while idx < len(Data):
        ...
        Attribute = (
            "%04x"
            % struct.unpack("H", struct.pack(">H", int(Data[idx : idx + 4], 16)))[0]
        )
        idx += 4
        buildPayload += Attribute

    Domoticz.Log(
        "buildframe_read_attribute_request - NwkId: %s Ep: %s ClusterId: %s nbAttribute: %s Data: %s"
...
def buildframe_write_attribute_response(
    frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
):
...
def decode_endian_data(data, datatype):
    if datatype in ("10", "18", "20", "28", "30"):
        ...
        value = data
        Domoticz.Log(
            "-------> Data not decoded Type: %s Value: %s " % (datatype, value)
        )
    return value


def buildframe_read_attribute_response(
    frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
):
    ...
    while idx < len(Data):
        ...
        if Status == "00":
            ...
            if DType in SIZE_DATA_TYPE:
                ...
                "41",
                "42",
            ):  # ZigBee_OctedString = 0x41, ZigBee_CharacterString = 0x42
    ...
    Domoticz.Log(
        "buildframe_read_attribute_response for 0x8100 - NwkId: %s Ep: %s ClusterId: %s nbAttribute: %s Data: %s from frame: %s"
        % (SrcNwkId, SrcEndPoint, ClusterId, nbAttribute, buildPayload, frame)
    )
...
def buildframe_report_attribute_response(
    frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
):
    buildPayload = Sqn + SrcNwkId + SrcEndPoint + ClusterId
    nbAttribute = 0
    ...
    Domoticz.Log(
        "buildframe_report_attribute_response - NwkId: %s Ep: %s ClusterId: %s nbAttribute: %s Data: %s"
        % (SrcNwkId, SrcEndPoint, ClusterId, nbAttribute, buildPayload)
    )
...
def buildframe_configure_reporting_response(
    frame, Sqn, SrcNwkId, SrcEndPoint, ClusterId, Data
):
    ...
    Domoticz.Log(
        "buildframe_configure_reporting_response - NwkId: %s Ep: %s ClusterId: %s nbAttribute: %s Data: %s"
        % (SrcNwkId, SrcEndPoint, ClusterId, nbAttribute, buildPayload)
    )
    newFrame = "01"  # 0:2
