...
class TestRetrieveWidgetidFromDeviceIdUnit(unittest.TestCase):
    ...
    def test_found(self):
        ...
        devices = _make_devices(("ieee-1", [(1, {})]))
        domo.load_list_of_domoticz_widget(obj, devices)
        idx = domo.retreive_widgetid_from_deviceId_unit(obj, devices, "ieee-1", 1)
        self.assertIsNotNone(idx)

    def test_not_found_returns_none(self):
        obj = _make_self()
        domo.load_list_of_domoticz_widget(obj, {})
        idx = domo.retreive_widgetid_from_deviceId_unit(obj, {}, "ghost", 99)
...
class TestFreeUnit(unittest.TestCase):

    def test_empty_device_returns_1(self):
        obj = _make_self()
        devices = _make_devices(("ieee-1", []))
        result = domo.FreeUnit(obj, devices, "ieee-1")
        self.assertEqual(result, 1)
    ...
    def test_new_device_id_returns_1(self):
        obj = _make_self()
        devices = {}
        result = domo.FreeUnit(obj, devices, "brand-new-ieee")
        self.assertEqual(result, 1)
    ...
    def test_consecutive_slots(self):
        ...
        result = domo.FreeUnit(obj, devices, "ieee-1", nbunit_=2)
        self.assertEqual(result, 6)

    def test_invalid_nbunit_returns_none(self):
        obj = _make_self()
        devices = _make_devices(("ieee-1", []))
        self.assertIsNone(domo.FreeUnit(obj, devices, "ieee-1", nbunit_=0))
        self.assertIsNone(domo.FreeUnit(obj, devices, "ieee-1", nbunit_=-1))
        self.assertIsNone(domo.FreeUnit(obj, devices, "ieee-1", nbunit_=300))

    def test_non_int_nbunit_returns_none(self):
        obj = _make_self()
        devices = _make_devices(("ieee-1", []))
        self.assertIsNone(domo.FreeUnit(obj, devices, "ieee-1", nbunit_="two"))
...
# ---------------------------------------------------------------------------
# 20. how_many_legacy_slot_available
# ---------------------------------------------------------------------------

class TestHowManyLegacySlotAvailable(unittest.TestCase):

    def test_empty_devices_returns_254(self):
        self.assertEqual(domo.how_many_legacy_slot_available({}), 254)

    def test_none_devices_returns_zero(self):
        self.assertEqual(domo.how_many_legacy_slot_available(None), 0)

    def test_two_occupied_slots(self):
        devices = {1: MagicMock(), 5: MagicMock()}
        self.assertEqual(domo.how_many_legacy_slot_available(devices), 252)
