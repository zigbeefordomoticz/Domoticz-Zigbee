...
"""Tools.check_and_remove_duplicates

Small helper script used during development to detect and optionally
remove duplicate installed distributions (multiple versions) inside a
given directory. This is useful when maintaining the repository tree or
packaging where multiple .dist-info directories for the same package
may appear and cause ambiguous imports.

The script provides the following functions:
- find_duplicates(directory): scan a path for distributions with the
    same package name and return a mapping package -> versions list.
- remove_duplicates(directory, duplicates, keep_version): remove
    specified duplicate .dist-info directories keeping a chosen
    version.
- main(): CLI wrapper to print or remove duplicates.

Usage (from shell):

        python Tools/check_and_remove_duplicates.py /path/to/site-packages

Pass --remove-duplicates to remove duplicates (careful: destructive).
"""
