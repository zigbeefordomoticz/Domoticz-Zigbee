...
def sendZigateCmd(cmd,length,datas) :
	...
	httpConn.Send(bytes.fromhex(str(lineinput))+bytes("\r\n",'utf-8'))	
...
def MajDomoDevice(Addr,Ep,Type,value) :
	...
	for x in Devices:
		if Devices[x].DeviceID == str(DeviceID) : 
			...
			if DType=="lumi.sensor_86sw1" or DType=="lumi.sensor_motion" or DType=="lumi.sensor_switch.aq2" or DType=="lumi.sensor_switch" or DType=="lumi.sensor_smoke" :  # detecteur de presence (v1) ou ionterrupteur Xiaomi
				if Type==Dtypename :
					if value == "01" :
						state="On"
					elif value == "00" :
