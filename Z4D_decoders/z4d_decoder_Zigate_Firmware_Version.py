def Decode8010(self, Devices, MsgData, MsgLQI):
    ...
    if len(MsgData) == 8:
        ...
        self.log.logging('Input', 'Debug', 'Decode8010 Major: %s Minor: %s Full: %s' % (self.FirmwareMajorVersion, FirmwareMinorVersion, self.FirmwareVersion))
    ...
    self.log.logging('Input', 'Debug', 'Decode8010 - Reception Version list:%s len: %s Branch: %s Major: %s Version: %s' % (MsgData, MsgLen, self.FirmwareBranch, self.FirmwareMajorVersion, self.FirmwareVersion))
    if self.FirmwareBranch in FIRMWARE_BRANCH:
        if int(self.FirmwareBranch) in (98, 99):
            self.log.logging('Input', 'Status', 'Untested Zigbee adaptater model. If this is a Sonoff USB Dongle E that is known issue, otherwise please report to the Zigbee for Domoticz team')
            ...
            if int(self.FirmwareMajorVersion, 16) == 3:
            ...
            elif int(self.FirmwareMajorVersion, 16) == 4:
                ...
                self.pluginParameters['CoordinatorFirmwareVersion'] = '%04x' % int(self.FirmwareVersion, 16)
            elif int(self.FirmwareMajorVersion, 16) == 5:
        ...
        self.ControllerData['Firmware Version'] = 'Branch: %s Major: %s Version: %s' % (self.FirmwareBranch, self.FirmwareMajorVersion, self.FirmwareVersion)
