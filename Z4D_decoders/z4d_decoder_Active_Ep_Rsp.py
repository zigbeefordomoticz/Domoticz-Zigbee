def Decode8045(self, Devices, MsgData, MsgLQI):
    ...
    self.log.logging('Pairing', 'Debug', 'Decode8045 - Reception Active endpoint response: SQN: %s Status: %s Short Addr: %s List: %s Ep List:  %s' % (MsgDataSQN, DisplayStatusCode(MsgDataStatus), MsgDataShAddr, MsgDataEpCount, MsgDataEPlist))
    ...
    for i in range(0, 2 * int(MsgDataEpCount, 16), 2):
        ...
        self.log.logging('Input', 'Status', '[%s] NEW OBJECT: %s Active Endpoint Response Ep: %s LQI: %s' % ('-', MsgDataShAddr, tmpEp, int(MsgLQI, 16)))
        if self.ListOfDevices[MsgDataShAddr]['Status'] != '8045':
            self.log.logging('Input', 'Log', '[%s] NEW OBJECT: %s/%s receiving 0x8043 while in status: %s' % ('-', MsgDataShAddr, tmpEp, self.ListOfDevices[MsgDataShAddr]['Status']))
