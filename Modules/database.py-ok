...
def LoadDeviceList(self):
    ...
    if self.pluginconf.pluginConf["useDomoticzDatabase"]:
        ...
        self.log.logging( "Database", "Debug", "Database from Dz is recent: %s Loading from Domoticz Db" % is_domoticz_recent(self, saving_time, _DeviceListFileName) )
        res = "Success"
...
def _read_DeviceList_Domoticz(self):
    ...
    self.log.logging( "Database", "Log", "Plugin data found on DZ with date %s" % (time.strftime("%A, %Y-%m-%d %H:%M:%S", time.localtime(time_stamp))), )
