    ...
    for nwkid in list(self.ListOfDevices):
        if 'Manufacturer Name' in self.ListOfDevices[nwkid]:                        
            if str(self.ListOfDevices[nwkid]['Manufacturer Name']).upper() in MANUFACTURER_NAME_TO_CODE:
                if self.ListOfDevices[nwkid]['Manufacturer'] != MANUFACTURER_NAME_TO_CODE[ str(self.ListOfDevices[nwkid]['Manufacturer Name']).upper() ]:
                    self.ListOfDevices[nwkid]['Manufacturer'] = MANUFACTURER_NAME_TO_CODE[ str(self.ListOfDevices[nwkid]['Manufacturer Name']).upper() ]
