...
def casaia_check_irPairing( self, NwkId):
    ...
    if 'CASA.IA' not in self.ListOfDevices[ NwkId ]:
        self.log.logging( "CasaIA", 'Error', "casaia_check_irPairing - %s No CASA.IA Attributes" %NwkId, NwkId,
            {'Error code': 'CASAIA-CHKPAIRING-01'})
        AC201_read_AC_status_request( self, NwkId)
        return None

    if DEVICE_ID not in self.ListOfDevices[NwkId]['CASA.IA']:
        self.log.logging( "CasaIA", 'Error', "casaia_check_irPairing - %s No DEVICE_ID Attributes" %NwkId, NwkId,
            {'Error code': 'CASAIA-CHKPAIRING-02'})
        AC201_read_AC_status_request( self, NwkId)
        return None

    if 'IRCode' not in self.ListOfDevices[NwkId]['CASA.IA'][DEVICE_ID]:
        self.log.logging( "CasaIA", 'Error', "casaia_check_irPairing - %s No IRCode Attributes" %NwkId, NwkId,
            {'Error code': 'CASAIA-CHKPAIRING-03'})
        AC201_read_AC_status_request( self, NwkId)
        return None

    if self.ListOfDevices[NwkId]['CASA.IA'][DEVICE_ID]['IRCode'] in ( 0x0, 0xffff):
        self.log.logging( "CasaIA", 'Error', "casaia_check_irPairing - %s IRCode %s not in place"\
            %(NwkId, self.ListOfDevices[NwkId]['CASA.IA'][DEVICE_ID]['IRCode']), NwkId,
            {'Error code': 'CASAIA-CHKPAIRING-04'})
        return None

    if 'ModuleIRCode' in self.ListOfDevices[NwkId]['CASA.IA'][DEVICE_ID] and\
        int(self.ListOfDevices[NwkId]['CASA.IA'][DEVICE_ID]['ModuleIRCode'],16) == int(self.ListOfDevices[NwkId]['CASA.IA'][DEVICE_ID]['IRCode'],16):
        return True
...
def casaia_ac201_ir_pairing( self, NwkId):
    ...
    if pac_code and int(pac_code,16) not in ( 0x0, 0xffff):
        ...
        self.log.logging( "Casaia", 'Error', "casaia_ac201_ir_pairing - %s IRCode %s not in place" %(NwkId, pac_code),
            NwkId,{'Error code': 'CASAIA-ACPAIRING-01'})
...
def AC211_ReadPairingCodeRequest( self, NwkId ):
    ...
    self.log.logging( "CasaIA", 'Debug', "AC211_read_multi_pairing_code_request ++++ %s payload: %s" %( NwkId, payload), NwkId)
...
def AC201_read_multi_pairing_code_request( self, NwkId ):
    ...
    self.log.logging( "CasaIA", 'Debug', "AC201_read_multi_pairing_code_request ++++ %s payload: %s" %( NwkId, payload), NwkId)
...
def AC201_write_multi_pairing_code_request( self, NwkId , pairing_code_value ):
    ...
    self.log.logging( "CasaIA", 'Debug', "AC201_write_multi_pairing_code_request ++++ %s payload: %s" %( NwkId, payload), NwkId)
...
def write_AC201_status_request( self, NwkId, Action, setpoint = None):
    ...
    if Action not in AC201_COMMAND:
        self.log.logging( "CasaIA", 'Error', "write_AC201_status_request - %s Unknow action: %s" %(NwkId,Action),
            NwkId,{'Error code': 'CASAIA-AC201STREQ-01'})
        return
    if Action in ( 'HeatSetpoint', 'CoolSetpoint') and setpoint is None:
        self.log.logging( "CasaIA", 'Error', "write_AC201_status_request - %s Setpoint without a setpoint value !" %(NwkId),
            NwkId,{'Error code': 'CASAIA-AC201STREQ-02'})
        return        
...
def AC201_read_learned_data_group_status_request( self, NwkId):
    ...
    raw_APS_request( self, NwkId, EPout, 'ffad', '0104', payload, zigate_ep=ZIGATE_EP)
    self.log.logging( "CasaIA", 'Debug', "AC201_read_learned_data_group_status_request ++++ %s/%s payload: %s" %( NwkId, EPout, payload), NwkId)
...
def AC201_read_learned_data_group_name_request(self, NwkId):
    ...
    self.log.logging( "CasaIA", 'Debug', "AC201_read_learned_data_group_name_request ++++ %s payload: %s" %( NwkId, payload), NwkId)
...
def AC201_read_AC_status_response( self, Devices, NwkId, Ep, payload):
    ...
    self.log.logging( "CasaIA", "Debug" , "read_AC_status_response Status: %s device_type: %s device_id: %s pairing_code: %s current_temp: %s system_mode: %s heat_setpoint: %s cool_setpoint: %s fan_mode: %s"
        %( status, device_type, device_id, pairing_code, current_temp, system_mode, heat_setpoint, cool_stepoint, fan_mode),NwkId)
    ...
    # Update Fan Mode
    self.log.logging( "CasaIA", "Debug" , "read_AC_status_response Status: %s request Update Fan Mode: %s" %( NwkId, fan_mode),NwkId)
    if system_mode == '00':
    ...
    if str(check_hot_cold_setpoint(self, NwkId)) == 'Heat':
        ...
        self.log.logging( "CasaIA", "Debug" , "read_AC_status_response Status: %s request Update Setpoint: %s" %( NwkId, setpoint),NwkId)
...
def get_casaia_attribute( self, NwkId, Attribute, device_id = None):
    ...
    if 'CASA.IA' not in self.ListOfDevices[ NwkId ]:
        self.log.logging( "CasaIA", "Debug" , "get_casaia_attribute - %s CASA.IA not found in %s" %(NwkId,self.ListOfDevices[ NwkId ]),NwkId)
        return None
    if device_id:
        if Attribute not in self.ListOfDevices[ NwkId ]['CASA.IA'][ device_id ]:
            self.log.logging( "CasaIA", "Debug" , "get_casaia_attribute (1) - %s Attribute: %s not found in %s" %(NwkId,self.ListOfDevices[ NwkId ]['CASA.IA'][device_id]),NwkId)
            return None
        return self.ListOfDevices[ NwkId ]['CASA.IA'][ device_id ][ Attribute ]
    if Attribute not in self.ListOfDevices[ NwkId ]['CASA.IA']:
        self.log.logging( "CasaIA", "Debug" , "get_casaia_attribute (2) - %s Attribute: %s not found in %s" %(NwkId,self.ListOfDevices[ NwkId ]['CASA.IA']),NwkId)
        return None
