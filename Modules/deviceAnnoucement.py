...
def device_annoucementv0( self, Devices, MsgData, MsgLQI  ):
    ...
    if MsgSrcAddr in self.ListOfDevices:
        if 'ZDeviceName' in self.ListOfDevices[MsgSrcAddr]:
            loggingPairing( self, 'Status', "Device Announcement: %s(%s, %s) Join Flag: %s LQI: %s ChangeShortID: %s " \
                    %( self.ListOfDevices[MsgSrcAddr]['ZDeviceName'], MsgSrcAddr, MsgIEEE, MsgRejoinFlag, int(MsgLQI,16), newShortId ))
        else:
    ...
    if DeviceExist(self, Devices, MsgSrcAddr, MsgIEEE):
        ...
        #            loggingInput( self, 'Log', "        -> ExpDeviceAnnoucement 2: droping packet for %s due to Main Powered and Live LQI: %s" \
