...
def tuyaReadRawAPS(self, Devices, NwkId, srcEp, ClusterID, dstNWKID, dstEP, MsgPayload):
    ...
    if cmd in ( "01", "02",):  # TY_DATAREQUEST, TY_DATA_RESPONE, TY_DATA_REPORT
...
def tuya_default_response(self, SrcNwkId, SrcEndPoint, ClusterID, Command, Sqn, fcf, manuf_code):
...
def tuya_backlight_command(self, nwkid, mode):
    if int(mode) in {0, 1, 2}:
        ...
        write_attribute( self, nwkid, ZIGATE_EP, "01", ONOFF_CLUSTER, "0000", "00", backlist_attribute, "30", "%02x" % int(mode), ackIsDisabled=False )
