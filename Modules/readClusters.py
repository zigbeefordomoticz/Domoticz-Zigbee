    ...
    if int(AttType, 16) == 0x42:  # CharacterString
        ...
        try:
            ...
            if handleErrors:  # If there is an error we force the result to '' This is used for 0x0000/0x0005
                self.log.logging(
                    "Cluster", "Log", "decodeAttribute - seems errors decoding %s, so returning empty" % str(Attribute)
                )
                decode = ""
            else:
                ...
                self.log.logging(
                    "Cluster",
                    "Debug",
                    "decodeAttribute - seems errors, returning with errors ignore From: %s to >%s<"
                    % (str(Attribute), str(decode)),
                )
...
def ReadCluster(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - %s - %s/%s AttrId: %s AttrType: %s Attsize: %s Status: %s AttrValue: %s"
    ...
    if MsgAttrStatus != "00" and MsgClusterId != "0500":
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - Status %s for addr: %s/%s on cluster/attribute %s/%s"
            % (MsgAttrStatus, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID),
            nwkid=MsgSrcAddr,
        )
...
def Cluster0000(
    ...
    if MsgAttrID == "0000":  # ZCL Version
        ...
        self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp][MsgClusterId][MsgAttrID] = str(
        ...
        self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp][MsgClusterId][MsgAttrID] = str(
            decodeAttribute(self, MsgAttType, MsgClusterData, handleErrors=True)
        )
        ...
        AttrModelName = decodeAttribute(
            self, MsgAttType, MsgClusterData[0:idx], handleErrors=True
        )  # In case there is an error while decoding then return ''
        ...
        if modelName == "TS011F":
            ...
            if "ZDeviceID" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr]["ZDeviceID"] == "0402":
                ...
                "ZDeviceID" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr]["ZDeviceID"] == "0008"
            ):
                # Range extender
                modelName += "-extender"
            else:
            ...
            self.log.logging(
                "Cluster",
                "Log",
                "ReadCluster - %s / %s - Recepion Model: >%s< ManufName: >%s<"
                % (MsgClusterId, MsgAttrID, modelName, manufacturer_name),
                MsgSrcAddr,
            )
            ...
            if manufacturer_name in TUYA_SIREN_MANUFACTURER:  # Sirene
                ...
                self.log.logging(
                    "Cluster", "Log", "ReadCluster - %s / %s force to 2 Gangs Switch" % (MsgSrcAddr, MsgSrcEp)
                )
                ...
                self.log.logging(
                    "Cluster", "Log", "ReadCluster - %s / %s force to Watering Timer" % (MsgSrcAddr, MsgSrcEp)
                )
            ...
            if "Manufacturer Name" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr][
                "Manufacturer Name"
            ] in ("_TYZB01_ncutbjdi",):
                # QS-Zigbee-S05-LN
        ...
        self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp][MsgClusterId][
            MsgAttrID
        ] = AttrModelName  # We store the original one
        self.log.logging(
            "Cluster",
            "Debug",
            ...
            for iterEp in list(self.ListOfDevices[MsgSrcAddr]["Ep"]):
                if "ClusterType" in list(self.ListOfDevices[MsgSrcAddr]["Ep"][iterEp]):
                    self.log.logging(
                        "Cluster",
                        "Debug",
                        "ReadCluster - %s / %s - %s %s is already provisioned in Domoticz"
        ...
        if (
            self.ListOfDevices[MsgSrcAddr]["Model"] == modelName
            and self.ListOfDevices[MsgSrcAddr]["Model"] in self.DeviceConf
        ):
            # This looks like a Duplicate, just drop
            self.log.logging(
                "Cluster", "Debug", "ReadCluster - %s / %s - no action" % (MsgClusterId, MsgAttrID), MsgSrcAddr
            )
            return
        ...
        if (
            ...
            self.log.logging(
                "Cluster",
                "Error",
                "ReadCluster - %s / %s - no action as it is a different Model Name than registered %s"
                % (MsgClusterId, MsgAttrID, modelName),
                MsgSrcAddr,
            )
            return
        ...
        if self.ListOfDevices[MsgSrcAddr]["Model"] in self.DeviceConf:  # If the model exist in DeviceConf.txt
            modelName = self.ListOfDevices[MsgSrcAddr]["Model"]
            self.log.logging(
                "Cluster", "Debug", "Extract all info from Model : %s" % self.DeviceConf[modelName], MsgSrcAddr
            )

            if (
                "ConfigSource" in self.ListOfDevices[MsgSrcAddr]
                and self.ListOfDevices[MsgSrcAddr]["ConfigSource"] == "DeviceConf"
            ):
            ...
            if "Type" in self.DeviceConf[modelName]:  # If type exist at top level : copy it
                ...
                if "Ep" in self.ListOfDevices[MsgSrcAddr]:
                    ...
                    del self.ListOfDevices[MsgSrcAddr][
                        "Ep"
                    ]  # It has been prepopulated by some 0x8043 message, let's remove them.
                    self.ListOfDevices[MsgSrcAddr][
                        "Ep"
                    ] = {}  # It has been prepopulated by some 0x8043 message, let's remove them.
                    self.log.logging(
                        "Cluster", "Debug", "-- Record removed 'Ep' %s" % (self.ListOfDevices[MsgSrcAddr]), MsgSrcAddr
                    )
            ...
            for Ep in self.DeviceConf[modelName]["Ep"]:  # For each Ep in DeviceConf.txt
                    ...
                    if _BackupEp and Ep in _BackupEp:
                        ...
                        for attr in _BackupEp[Ep][cluster]:
                            if attr in self.ListOfDevices[MsgSrcAddr]["Ep"][Ep][cluster]:
                                if (
                                    self.ListOfDevices[MsgSrcAddr]["Ep"][Ep][cluster][attr] == ""
                                    or self.ListOfDevices[MsgSrcAddr]["Ep"][Ep][cluster][attr] == {}
                                ):
                                    self.ListOfDevices[MsgSrcAddr]["Ep"][Ep][cluster][attr] = _BackupEp[Ep][cluster][
                                        attr
                                    ]
                            else:
                ...
                if "ColorMode" in self.DeviceConf[modelName]["Ep"][Ep]:
                    ...
                    if "ColorMode" in self.DeviceConf[modelName]["Ep"][Ep]:
                        self.ListOfDevices[MsgSrcAddr]["ColorInfos"]["ColorMode"] = int(
                            self.DeviceConf[modelName]["Ep"][Ep]["ColorMode"]
                        )
        ...
        if legrand:
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "%s/%s ReadCluster - 0x0000 - Operating Time: %sdays %shours %smin %ssec"
                % (MsgSrcAddr, MsgSrcEp, dd, hh, mm, ss),
                MsgSrcAddr,
            )
        else:
            self.log.logging(
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - 0x0000 - %s/%s Attribut %s %s %s %s"
        ...
        self.log.logging(
            "Cluster",
            "Log",
            "readCluster - %s - %s/%s unknown attribute: %s %s %s %s "
...
def Cluster0001(
    ...
    if MsgAttrID == "0000":  # Voltage
        ...
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s General Voltage: %s V " % (MsgSrcAddr, value), MsgSrcAddr
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster 0001 %s Alarm Mask: UnderVoltage: %s OverVoltage: %s MainPowerLost: %s"
        ...
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s Battery Voltage: %s " % (MsgSrcAddr, value), MsgSrcAddr
        )
        MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, str(value))
        ...
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s Battery Percentage: %s " % (MsgSrcAddr, value), MsgSrcAddr
        )
        ...
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s Battery size: %s " % (MsgSrcAddr, value), MsgSrcAddr
        )

    elif MsgAttrID == "0033":  # Battery Quantity
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s Battery Quantity: %s " % (MsgSrcAddr, value), MsgSrcAddr
        )

    elif MsgAttrID == "0035":  # Battery Alarm Mask
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s Attribut 0035: %s " % (MsgSrcAddr, value), MsgSrcAddr
        )

    elif MsgAttrID == "0036":  # Minimum Threshold
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s Minimum Threshold: %s " % (MsgSrcAddr, value), MsgSrcAddr
        )
        ...
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "readCluster 0001 - %s Cluster Version: %s " % (MsgSrcAddr, value), MsgSrcAddr
        )
...
def UpdateBatteryAttribute(self, Devices, MsgSrcAddr, MsgSrcEp):
    ...
    if (
        "0020" in self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp]["0001"]
        and self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp]["0001"]["0020"] != {}
    ):
        battRemainingVolt = float(self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp]["0001"]["0020"])

    if (
        "0021" in self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp]["0001"]
        and self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp]["0001"]["0021"] != {}
    ):
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "readCluster 0001 - Device: %s Model: %s mainVolt:%s , battVolt:%s, battRemainingVolt: %s, battRemainPer:%s "
        % (MsgSrcAddr, self.ListOfDevices[MsgSrcAddr]["Model"], mainVolt, battVolt, battRemainingVolt, battRemainPer),
        MsgSrcAddr,
    )
    ...
    if value:
        ...
        if value != self.ListOfDevices[MsgSrcAddr]["Battery"]:
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster 0001 - Device: %s Model: %s Updating battery to %s"
                % (MsgSrcAddr, self.ListOfDevices[MsgSrcAddr]["Model"], value),
                MsgSrcAddr,
            )
...
def Cluster0003(
    ...
    checkAndStoreAttributeValue(
        self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, str(decodeAttribute(self, MsgAttType, MsgClusterData))
    )
    ...
    if MsgAttrID == "0000":  # IdentifyTime Attribute
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster %s - %s/%s Remaining time to identify itself %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, int(MsgClusterData, 16)),
        )
...
def Cluster0005(
    ...
    if MsgAttrID == "0000":  # SceneCount
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s Scene Count: %s %s %s %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s Scene Cuurent Scene: %s %s %s %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s Scene Valid : %s %s %s %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s Scene NameSupport: %s %s %s %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s Scene Last Configured By : %s %s %s %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
...
def Cluster0006(
    ...
    if "Model" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr]["Model"] in ("AC211", "AC221"):
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
        return
    ...
    if MsgAttrID in ("0000", "8000"):
        ...
        if self.ListOfDevices[MsgSrcAddr]["Model"] == "lumi.ctrl_neutral1" and MsgSrcEp != "02":
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - ClusterId=%s - Unexpected EP, %s/%s MsgAttrID: %s, MsgAttType: %s, MsgAttSize: %s, Value: %s"
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
                MsgSrcAddr,
            )
            return
        ...
        if self.ListOfDevices[MsgSrcAddr]["Model"] == "lumi.ctrl_neutral2" and MsgSrcEp != "02" and MsgSrcEp != "03":
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - ClusterId=%s - not processed EP, %s/%s MsgAttrID: %s, MsgAttType: %s, MsgAttSize: %s, Value: %s"
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
                MsgSrcAddr,
            )
            return
        ...
        if self.ListOfDevices[MsgSrcAddr]["Model"] == "TS0601-Parkside-Watering-Timer":
            self.log.logging(
                "Cluster",
                "Log",
                "ReadCluster - ClusterId=0006 - %s/%s MsgAttrID: %s, MsgAttType: %s, MsgAttSize: %s, : %s"
                % (MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
                MsgSrcAddr,
            )
            MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgClusterData)
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - Feedback from device %s/%s Attribute 0xf000 value: %s-%s"
            % (MsgSrcAddr, MsgSrcEp, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        if _Xiaomi_code in XIAOMI_CODE:
            if "ZDeviceName" in self.ListOfDevices[MsgSrcAddr]:
                ...
                self.log.logging(
                    "Cluster",
                    "Debug",
                    "ReadCluster - Xiaomi 0006/f000 - %s/%s %s: %s"
                    % (MsgSrcAddr, MsgSrcEp, XIAOMI_CODE[_Xiaomi_code], int(_Xiaomi_Value, 16)),
                    MsgSrcAddr,
                )
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - Xiaomi 0006/f000 - - %s/%s Unknown Xiaomi Code %s raw data: %s (please report to @pipiche)"
                % (MsgSrcAddr, MsgSrcEp, _Xiaomi_code, MsgClusterData),
                MsgSrcAddr,
            )
...
def Cluster0008(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - ClusterID: %s Addr: %s MsgAttrID: %s MsgAttType: %s MsgAttSize: %s MsgClusterData: %s"
    ...
    if MsgAttrID == "0000":  # Current Level
        if (
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - ClusterId=0008 - %s/%s MsgAttrID: %s, MsgAttType: %s, MsgAttSize: %s, : %s"
                % (MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
                MsgSrcAddr,
            )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - ClusterId=0008 - %s/%s Attr: %s Value: %s"
...
def Cluster0009(
    ...
    self.log.logging(
        "Cluster",
        "Log",
        "ReadCluster 0101 - Dev: %s, EP:%s AttrID: %s, AttrType: %s, AttrSize: %s Attribute: %s Len: %s"
        % (MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, len(MsgClusterData)),
        MsgSrcAddr,
    )
...
def Cluster000c(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - ClusterID=000C - MsgSrcEp: %s MsgAttrID: %s MsgAttType: %s MsgClusterData: %s "
        % (MsgSrcEp, MsgAttrID, MsgAttType, MsgClusterData),
        MsgSrcAddr,
    )
    checkAndStoreAttributeValue(
        self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, str(decodeAttribute(self, MsgAttType, MsgClusterData))
    ...
    if MsgAttrID == "001c":  # Description
        self.log.logging(
            "Cluster", "Debug", "%s/%s Description: %s" % (MsgSrcAddr, MsgSrcEp, MsgClusterData), MsgSrcAddr
        )

    elif MsgAttrID == "0051":  #
        self.log.logging(
            "Cluster", "Debug", "%s/%s Out of service: %s" % (MsgSrcAddr, MsgSrcEp, MsgClusterData), MsgSrcAddr
        )
        ...
        if getEPforClusterType(self, MsgSrcAddr, "Analog") and MsgAttType == "39":
            ...
            self.log.logging(
                "Cluster",
                "Log",
                "readCluster - %s - %s/%s Xiaomi attribute: %s:  %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, decodeAttribute(self, MsgAttType, MsgClusterData)),
                MsgSrcAddr,
            )
            MajDomoDevice(
                self,
        ...
        if len(EPforPower) == len(EPforMeter) == len(EPforPowerMeter) == 0:
            ...
            self.log.logging(
                "Cluster", "Debug", "ReadCluster - ClusterId=000c - Magic Cube angle: %s" % rotation_angle, MsgSrcAddr
            )
            MajDomoDevice(
                self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, str(int(rotation_angle)), Attribute_="0055"
            )
            if rotation_angle < 0:
                # anti-clokc
            ...
            len(EPforPower) > 0 or len(EPforMeter) > 0 or len(EPforPowerMeter) > 0
        ):  # We have several EPs in Power/Meter
            value = round(float(decodeAttribute(self, MsgAttType, MsgClusterData)), 3)
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - ClusterId=000c - MsgAttrID=0055 - on Ep "
                + str(MsgSrcEp)
                + " reception Conso Prise Xiaomi: "
                + str(value),
                MsgSrcAddr,
            )
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - ClusterId=000c - List of Power/Meter EPs"
                + str(EPforPower)
                + str(EPforMeter)
                + str(EPforPowerMeter),
                MsgSrcAddr,
        ...
        MsgAttrID == "006f"
    ):  # Status flag ( Bit 0 = IN ALARM, Bit 1 = FAULT, Bit 2 = OVERRIDDEN, Bit 3 = OUT OF SERVICE )
        ...
        self.log.logging(
            "Cluster", "Debug", "ReadCluster - ClusterId=000c - Magic Cube Rotation: " + str(MsgClusterData), MsgSrcAddr
        )
...
def Cluster000f(
    ...
    if MsgAttrID == "0051":
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster %s - %s/%s Model: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, self.ListOfDevices[MsgSrcAddr]["Model"]),
            MsgSrcAddr,
        )
        if self.ListOfDevices[MsgSrcAddr]["Model"] != {}:
            if self.ListOfDevices[MsgSrcAddr]["Model"] in LEGRAND_REMOTE_SWITCHS:
                self.log.logging(
                    "Cluster", "Debug", "Legrand remote Switch Present Value: %s" % MsgClusterData, MsgSrcAddr
                )
        ...
        if MsgClusterData in STATUS_FLAGS:
            ...
            if MsgClusterData != "00":
                Domoticz.Status(
                    "Device %s/%s Status flag: %s %s"
                    % (MsgSrcAddr, MsgSrcEp, MsgClusterData, STATUS_FLAGS[MsgClusterData])
                )
            else:
                self.log.logging(
                    "Cluster",
...
def Cluster0012(
    ...
    if _modelName in ("lumi.remote.b686opcn01", "lumi.remote.b486opcn01", "lumi.remote.b286opcn01"):
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - ClusterId=0012 - Switch Aqara lumi.ctrl_ln2.aq1: EP: %s Attr: %s Value: %s "
            % (MsgSrcEp, MsgAttrID, value),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        ...
        checkAndStoreAttributeValue(
            self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, cube_decode(self, MsgClusterData, MsgSrcAddr)
        )
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - ClusterId=0012 - reception Xiaomi Magic Cube Value: "
            + str(cube_decode(self, MsgClusterData, MsgSrcAddr)),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Log",
            "readCluster - %s - %s/%s unknown attribute: %s %s %s %s Model: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, _modelName),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
...
def Cluster0100(
    ...
    if MsgAttrID == "0000":
        self.log.logging(
            "Cluster", "Debug", "ReadCluster 0100 - Shade Config: PhysicalClosedLimit: %s" % MsgClusterData, MsgSrcAddr
        )
    elif MsgAttrID == "0001":
        self.log.logging(
            "Cluster", "Debug", "ReadCluster 0100 - Shade Config: MotorStepSize: %s" % MsgClusterData, MsgSrcAddr
        )
    elif MsgAttrID == "0002":
        self.log.logging("Cluster", "Debug", "ReadCluster 0100 - Shade Config: Status: %s" % MsgClusterData, MsgSrcAddr)
    elif MsgAttrID == "0010":
        self.log.logging(
            "Cluster", "Debug", "ReadCluster 0100 - Shade Config: ClosedLimit: %s" % MsgClusterData, MsgSrcAddr
        )
    elif MsgAttrID == "0011":
...
def Cluster0101(
        ...
        if MsgClusterData == "01":
            ...
            if "ZDeviceName" in self.ListOfDevices[MsgSrcAddr]:
                self.log.logging(
                    "Cluster",
                    "Status",
                    "%s DoorLock state %s (%s)"
                    % (self.ListOfDevices[MsgSrcAddr]["ZDeviceName"], MsgClusterData, LOCKSTATE[MsgClusterData]),
                    MsgSrcAddr,
                )
    ...
    # Aqara related
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster %s/%s -  Vibration Strenght: %s %s %s"
            % (MsgClusterId, MsgAttrID, MsgClusterData, value, strenght),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            " ReadCluster %s/%s - AttrType: %s AttrLenght: %s AttrData: %s Vibration ==> angleX: %s angleY: %s angleZ: %s"
            % (MsgClusterId, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, angleX, angleY, angleZ),
            MsgSrcAddr,
        )
        MajDomoDevice(
            self,
...
def Cluster0102(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - %s - %s/%s - Attribute: %s, Type: %s, Size: %s Data: %s-%s"
    ...
    if MsgAttrID == "0000":
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Window Covering Type: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        WINDOW_COVERING = {
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Physical close limit lift cm: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Physical close limit Tilt cm: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Curent position Lift in cm: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Curent position Tilt in cm: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Number of Actuations – Lift: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Number of Actuations – Tilt: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            ...
            self,
            "Debug",
            "ReadCluster - %s - %s/%s - Config Status: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster 0x%s - %s - %s/%s - Current position lift in %%: %s, Type: %s, Size: %s Data: %s-%s"
            % (Source, MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        if "Model" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr]["Model"] != {}:
            ...
            self.log.logging(
                ...
                "Debug",
                "ReadCluster - %s - %s/%s - Model: %s"
                % (MsgAttrID, MsgSrcAddr, MsgSrcEp, self.ListOfDevices[MsgSrcAddr]["Model"]),
                MsgSrcAddr,
            )
            ...
            if self.ListOfDevices[MsgSrcAddr]["Model"] in ("TS0302", "1GANGSHUTTER1"):
                ...
                # Value: 50  -> Stopped
                if (
                    "Param" in self.ListOfDevices[MsgSrcAddr]
                    and "netatmoInvertShutter" in self.ListOfDevices[MsgSrcAddr]["Param"]
                    and self.ListOfDevices[MsgSrcAddr]["Param"]["netatmoInvertShutter"]
                ):
                    self.log.logging(
                        "Cluster",
                        "Debug",
                        "ReadCluster - %s - %s/%s - Model: %s ==>INVERSE==="
                        % (MsgAttrID, MsgSrcAddr, MsgSrcEp, self.ListOfDevices[MsgSrcAddr]["Model"]),
                        MsgSrcAddr,
                    )
                    value = 100 - value
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Shutter switch with neutral After correction value: %s"
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Curent position Tilte in %%: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Open limit lift cm: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Closed limit lift cm: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Velocity lift: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - Windows Covering mode: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Log",
            "ReadCluster - %s - %s/%s - Tuya Window Cover Status: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s - AttributeID: %s, Type: %s, Size: %s Data: %s-%s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
            MsgSrcAddr,
        )
...
def Cluster0201(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - 0201 - %s/%s AttrId: %s AttrType: %s AttSize: %s Data: %s"
        % (MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
        MsgSrcAddr,
    )
    ...
    if MsgAttrID == "0000":  # Local Temperature (Zint16)
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s Min Heat Setpoint Limit: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s Max Heat Setpoint Limit: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s Min Cool Setpoint Limit: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s Max Cool Setpoint Limit: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - %s/%s HVAC System Type Config: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
        ...
        ValueTemp = round(int(value) / 100, 2)
        self.log.logging(
            "Cluster", "Debug", "ReadCluster - 0201 - Heating Setpoint: %s ==> %s" % (value, ValueTemp), MsgSrcAddr
        )
            ...
            if self.ListOfDevices[MsgSrcAddr]["Model"] == "AC201A":
                ...
                if MsgClusterId in self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp]:
                    if "001c" in self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp][MsgClusterId]:
                        if self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp][MsgClusterId]["001c"] == 0x04:
                            MajDomoDevice(
                                self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, ValueTemp, Attribute_=MsgAttrID
                            )
                ...
                if "Schneider" in self.ListOfDevices[MsgSrcAddr]:
                    if "Target SetPoint" in self.ListOfDevices[MsgSrcAddr]["Schneider"]:
                        if self.ListOfDevices[MsgSrcAddr]["Schneider"]["Target SetPoint"] == int(
                            ((ValueTemp * 100) * 2) / 2
                        ):
                ...
                self.log.logging(
                    "Cluster",
                    "Debug",
                    "ReadCluster - 0201 - Request update on Domoticz %s not a Schneider, not a Eurotronics"
                    % MsgSrcAddr,
                    MsgSrcAddr,
                )
                MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, ValueTemp, Attribute_=MsgAttrID)
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster %s - %s/%s Control Sequence Operation: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
        ...
        if (
            ...
            and self.ListOfDevices[MsgSrcAddr]["Model"] in ("AC211", "AC221")
        ):
        ...
        if MsgAttrID == "e010":  # Schneider Thermostat Mode
            ...
            if MsgClusterData in THERMOSTAT_MODE:
                self.log.logging(
                    "Cluster",
                    "Debug",
                    "readCluster - %s - %s/%s Schneider Thermostat Mode %s "
                    % (MsgClusterId, MsgSrcAddr, MsgSrcEp, THERMOSTAT_MODE[MsgClusterData]),
                    MsgSrcAddr,
                )
            else:
                self.log.logging(
                    "Cluster",
                    "Debug",
                    "readCluster - %s - %s/%s Schneider Thermostat Mode 0xe010 %s "
                    % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                    MsgSrcAddr,
                )
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider ATTRIBUTE_THERMOSTAT_HACT_CONFIG  %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
            MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, "0201", MsgClusterData, Attribute_=MsgAttrID)
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider ATTRIBUTE_THERMOSTAT_OPEN_WINDOW_DETECTION_STATUS  %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
            checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider ATTRIBUTE_THERMOSTAT_OPEN_WINDOW_DETECTION_THRESHOLD  %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
            checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider ATTRIBUTE_THERMOSTAT_OPEN_WINDOW_DETECTION_INTERVAL  %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
            checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider FIP mode  %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
            checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider Valve Position  %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
            checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider Valve Calibration Status %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
            checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData)
...
def Cluster0202(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - 0202 - %s/%s AttrId: %s AttrType: %s AttSize: %s Data: %s"
        % (MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
        MsgSrcAddr,
    )
...
def Cluster0204(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster 0204 - Addr: %s Ep: %s AttrId: %s AttrType: %s AttSize: %s Data: %s"
        % (MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
        MsgSrcAddr,
    )
    ...
    if MsgAttrID == "0000":
        ...
        if MsgClusterData == "00":
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster %s/%s 0204 - Temperature Display Mode : %s --> °C"
                % (MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
        elif MsgClusterData == "01":
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster %s/%s 0204 - Temperature Display Mode : %s -->  °F"
                % (MsgSrcAddr, MsgSrcEp, MsgClusterData),
                MsgSrcAddr,
            )
...
def Cluster0300(
    ...
    if MsgAttrID == "0000":  # CurrentHue
        ...
        self.log.logging(
            "Cluster", "Debug", "ReadCluster0300 - %s/%s RemainingTime: %s" % (MsgSrcAddr, MsgSrcEp, value), MsgSrcAddr
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s Color Mode: %s %s %s %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s Attribute: %s %s %s %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
...
def Cluster0301(
    ...
    if MsgAttrID == "0010":  # Min
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s  Ballast Configuration Min Level %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "readCluster - %s - %s/%s  Ballast Configuration Max Level %s "
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData),
            MsgSrcAddr,
        )
...
def Cluster0400(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "readCluster - %s - %s/%s  Attr: %s Type: %s Size: %s %s "
        % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
        MsgSrcAddr,
    )
    value = int(decodeAttribute(self, MsgAttType, MsgClusterData))
    ...
    if (
        "Model" in self.ListOfDevices[MsgSrcAddr]
        and str(self.ListOfDevices[MsgSrcAddr]["Model"]).find("lumi.sensor") != -1
    ):
    ...
    if MsgAttrID == "0000" and MsgClusterData != "":
        ...
        if value > 0x7FFF and value < 0x954D:
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Invalid Temperature Measurement: %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, value),
                MsgSrcAddr,
            )
        else:
...
def Cluster0403(
    ...
    value = int(decodeAttribute(self, MsgAttType, MsgClusterData))
    self.log.logging(
        "Cluster", "Debug", "Cluster0403 - decoded value: from:%s to %s" % (MsgClusterData, value), MsgSrcAddr
    )
    ...
    if MsgAttrID == "0000":  # Atmo in mb
        ...
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging(
            "Cluster", "Debug", "ReadCluster - ClusterId=0403 - 0000 reception atm: " + str(value), MsgSrcAddr
        )
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s/%s ClusterId=%s - Scaled value %s: %s "
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s/%s ClusterId=%s - Scale %s: %s "
            % (MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgClusterData),
            MsgSrcAddr,
        )
...
def Cluster0406(
    ...
    if MsgAttrID == "0000":
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - %s - NwkId: %s Ep: %s AttrId: %s AttyType: %s Attsize: %s AttrValue: %s"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
        self.log.logging(
            "Cluster", "Debug", "ReadCluster - ClusterId=0406 - Sensor Type: " + str(MsgClusterData), MsgSrcAddr
        )
        ...
        self.log.logging(
            "Cluster", "Debug", "ReadCluster - ClusterId=0406 - Attribut 0030: " + str(MsgClusterData), MsgSrcAddr
        )
...
def Cluster0500(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster0500 - Security & Safety IAZ Zone - Device: %s MsgAttrID: %s MsgAttType: %s MsgAttSize: %s MsgClusterData: %s"
    ...
    if MsgAttrID == "0000":  # ZoneState ( 0x00 Not Enrolled / 0x01 Enrolled )
        ...
        if MsgClusterData != "" and MsgAttType in ("19", "21"):
            ...
            if (
                "IAS" in self.ListOfDevices[MsgSrcAddr]
                and MsgSrcEp in self.ListOfDevices[MsgSrcAddr]["IAS"]
                and "ZoneStatus" in self.ListOfDevices[MsgSrcAddr]["IAS"][MsgSrcEp]
            ):
    ...
    self.log.logging(
        "Cluster", "Debug", "ReadCluster0500 - Device: %s Data: %s" % (MsgSrcAddr, MsgClusterData), MsgSrcAddr
    )
...
def Cluster0502(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster0502 - Security & Safety IAZ Zone - Device: %s MsgAttrID: %s MsgAttType: %s MsgAttSize: %s MsgClusterData: %s"
        % (MsgSrcAddr, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
        MsgSrcAddr,
    )
    ...
    if MsgAttrID == "0000":  # Max Duration
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - 0502 - %s/%s Max Duration: %s"
        ...
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster - 0502 - %s/%s unknown attribute: %s %s %s %s"
            % (MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
            MsgSrcAddr,
        )
...
def compute_conso(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, raw_value):
    ...
    if (
        MsgSrcEp in self.ListOfDevices[MsgSrcAddr]["Ep"]
        ...
        self.log.logging(
            "Cluster", "Debug", "compute_conso - %s Power %s, div: %s --> %s Watts" % (MsgAttrID, conso, diviser, value)
        )
    elif (
        MsgSrcEp in self.ListOfDevices[MsgSrcAddr]["Ep"]
        and MsgClusterId in self.ListOfDevices[MsgSrcAddr]["Ep"][MsgSrcEp]
...
def Cluster0702( self, Devices, MsgSQN, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, Source ):
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "Cluster0702 - MsgAttrID: %s MsgAttType: %s DataLen: %s Data: %s decodedValue: %s"
        % (MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
        MsgSrcAddr,
    )
    ...
    if MsgAttrID == "0000":  # CurrentSummationDelivered
        if "Model" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr]["Model"] == 'ZLinky_TIC':
            ...
            if value > 0x7FFFFFFFFFFFFFFF:
                ...
                return
            self.log.logging( "Cluster", "Debug", "Cluster0702 - 0x0000 CURRENT_SUMMATION_DELIVERED Value: %s Conso: %s " % (MsgClusterData, conso), MsgSrcAddr, )
        ...
        self.log.logging(
            "Cluster", "Debug", "Cluster0702 - %s/%s Unit of Measure: %s" % (MsgSrcAddr, MsgSrcEp, value), MsgSrcAddr
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)

    elif MsgAttrID == "0301":  # Multiplier
        self.log.logging(
            "Cluster", "Debug", "Cluster0702 - %s/%s Multiplier: %s" % (MsgSrcAddr, MsgSrcEp, value), MsgSrcAddr
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)

    elif MsgAttrID == "0302":  # Divisor
        self.log.logging(
            "Cluster", "Debug", "Cluster0702 - %s/%s Divisor: %s" % (MsgSrcAddr, MsgSrcEp, value), MsgSrcAddr
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        ...
        self.log.logging(
            "Cluster", "Debug", "Cluster0702 - %s/%s TotalActivePower: %s" % (MsgSrcAddr, MsgSrcEp, value), MsgSrcAddr
        )
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        ...
        if MsgAttrID in ELECTRICAL_MEASURES:
            self.log.logging(
                "Cluster",
                "Debug",
                "Cluster0702 - %s/%s Schneider %s : %s "
                % (MsgSrcAddr, MsgSrcEp, ELECTRICAL_MEASURES[MsgAttrID], value),
                MsgSrcAddr,
            )
            checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        else:
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s Schneider Attribute: %s  Raw Data: %s Decoded Data: %s "
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgClusterData, value),
                MsgSrcAddr,
            )
        ...
        if MsgAttrID in ("2000", "2001", "2002"):  # Lx phase Power
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Power %s"
                % (MsgClusterId, MsgSrcAddr, fake_ep, line, conso),
            )
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Voltage %s"
                % (MsgClusterId, MsgSrcAddr, fake_ep, line, value),
            )
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Current %s"
                % (MsgClusterId, MsgSrcAddr, fake_ep, line, value),
            )
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Summation Power %s"
                % (MsgClusterId, MsgSrcAddr, fake_ep, line, conso),
            )
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "readCluster - %s - %s/%s CASAIA PC321 phase Power Clamp: %s %s %s %s (value: %s)"
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, value),
                MsgSrcAddr,
            )
...
def Cluster0b01(self, Devices, MsgSQN, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData, Source):
    ...
    if MsgAttrID == '000d':
        if "Model" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr]["Model"] == 'ZLinky_TIC':
            # Current Subscribed
            self.log.logging( "Cluster", "Debug", "Cluster0b01 - Simulate 45A Subscription" , MsgSrcAddr)
            value = 45  
            value = int(value)
    checkAndStoreAttributeValue( self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value,)
    ...
    if MsgAttrID in "050b":  # Active Power
        if -32768 <= int(MsgClusterData[0:4], 16) <= 32767:
            ...
            self.log.logging(
                "Cluster",
                "Log",
                "ReadCluster %s - %s/%s Attribute: %s Type: %s Size: %s Data: %s Out of Range!!"
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
                MsgSrcAddr,
            )
        ...
        value = int(decodeAttribute(self, MsgAttType, MsgClusterData))
        self.log.logging(
            "Cluster", "Debug", "ReadCluster %s - %s/%s Voltage %s" % (MsgClusterId, MsgSrcAddr, MsgSrcEp, value)
        )
        if value == 0xFFFF:
            return
        ...
        self.log.logging( "Cluster", "Debug", "ReadCluster %s - %s/%s Current %s" % (MsgClusterId, MsgSrcAddr, MsgSrcEp, value), MsgSrcAddr )
        value /= 100
        ...
        if "Model" in self.ListOfDevices[MsgSrcAddr] and self.ListOfDevices[MsgSrcAddr]["Model"] == "TS0121":
            ...
            self.log.logging( "Cluster", "Log", "ReadCluster %s - %s/%s SIMULATE 3 PHASES - TO BE REMOVED" % (MsgClusterId, MsgSrcAddr, MsgSrcEp), MsgSrcAddr )

            MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, str(randrange( 0x0, 0x3c) ), Attribute_='0908')
            MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, str(randrange( 0x0, 0x3c) ), Attribute_='0a08')
        ...
        if value == 0xffff:
            return
        checkAndStoreAttributeValue(self, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, value)
        self.log.logging( "Cluster", "Log", "ReadCluster %s - %s/%s Apparent Power %s" % (MsgClusterId, MsgSrcAddr, MsgSrcEp, value), MsgSrcAddr )
...
def zlinky_check_alarm( self, Devices, MsgSrcAddr, MsgSrcEp, value):
    ...
    if value == 0:
        return     
    if ( '0b01' in self.ListOfDevices[ MsgSrcAddr ]['Ep'][ '01' ] 
            and '000d' in self.ListOfDevices[ MsgSrcAddr ]['Ep'][ '01' ][ '0b01'] 
            and isinstance( self.ListOfDevices[ MsgSrcAddr ]['Ep'][ '01' ][ '0b01']['000d'], int)
    ):
        ...
        self.log.logging(
            "Cluster",
            "Log",
            "zlinky_check_alarm - %s/%s For TEST ONLY ISousc is set to 45 !!!! TO BE REMOVED"
            % ( MsgSrcAddr, MsgSrcEp),
            MsgSrcAddr,
            )
        if Isousc == 0:
        ...
        self.log.logging( "Cluster", "Log", "zlinky_check_alarm - %s/%s flevel- %s %s %s" % ( MsgSrcAddr, MsgSrcEp, value, Isousc, flevel ), MsgSrcAddr, )

        if flevel > 80:
            MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, "0009", "01", Attribute_="0005")
            self.log.logging( "Cluster", "Log", "zlinky_check_alarm - %s/%s Alarm-01" % ( MsgSrcAddr, MsgSrcEp ), MsgSrcAddr, )
        elif flevel > 90:
            MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, "0009", "02", Attribute_="0005")
            self.log.logging( "Cluster", "Log", "zlinky_check_alarm - %s/%s Alarm-02" % ( MsgSrcAddr, MsgSrcEp ), MsgSrcAddr, )
        elif flevel > 98:
            MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, "0009", "03", Attribute_="0005")
            self.log.logging( "Cluster", "Log", "zlinky_check_alarm - %s/%s Alarm-03" % ( MsgSrcAddr, MsgSrcEp ), MsgSrcAddr, )
    else:
        self.log.logging(
            "Cluster",
            "Log",
            "zlinky_check_alarm - %s/%s no Subscription found !!!!"
            % ( MsgSrcAddr, MsgSrcEp),
            MsgSrcAddr,
        )
...
def Clusterfc00(
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - %s - %s/%s MsgAttrID: %s, MsgAttType: %s, MsgAttSize: %s, : %s"
        % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData),
        MsgSrcAddr,
    )
    ...
    self.log.logging(
        "Cluster",
        "Debug",
        "ReadCluster - %s - %s/%s - past OnOff: %s, Lvl: %s"
        % (MsgClusterId, MsgSrcAddr, MsgSrcEp, onoffValue, lvlValue),
        MsgSrcAddr,
    )
    if MsgAttrID == "0001":  # On button
        self.log.logging(
        ...
        if action in ("00"):  # Short press
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - %s - %s/%s - DIM Release after %s seconds"
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, round(duration / 10)),
                MsgSrcAddr,
            )
            ...
            self.log.logging(
                "Cluster",
                "Debug",
                "ReadCluster - %s - %s/%s - DIM Action: %s not processed"
                % (MsgClusterId, MsgSrcAddr, MsgSrcEp, action),
                MsgSrcAddr,
            )
    ...
    checkAndStoreAttributeValue(
        self, MsgSrcAddr, MsgSrcEp, MsgClusterId, "0000", "%s;%s;%s" % (sonoffValue, slvlValue, sduration)
    )
    self.log.logging(
        "Cluster",
        "Debug",
...
def Clusterfc21(
    ...
    if MsgAttrID == "0001":
        self.log.logging(
            "Cluster",
            "Debug",
            "ReadCluster %s - %s/%s Orientation BSO: %s - %s °"
            % (MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData, int(MsgClusterData, 16)),
            MsgSrcAddr,
        )
        MajDomoDevice(self, Devices, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgClusterData)
