...
def UpdateBatteryAttribute( self, MsgSrcAddr, MsgSrcEp ):
    ...
    BATTERY_200PERCENT = ( 'PIR323-A', 'PIR323', 'DWS312-E', 'DWS312', "TS0207-waterleak", "Danalock V3", "V3-BTZB", "SML001" , "RWL021", "SPZB0001", "WarningDevice" , "SmokeSensor-N", "SmokeSensor-EM",
                           "SMOK_V16", "RH3001" ,"TS0201", "COSensor-N", "COSensor-EM" , "TS0043", "TH01", "66666", "DS01", "DSO1", "WB01", "WB-01", "TS0041")
...
def Cluster0201( self, Devices, MsgSQN, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData , Source):
    ...
    if MsgAttrID =='0000':  # Local Temperature (Zint16)
        ...
        self.log.logging( "Cluster", 'Log', "ReadCluster - %s - %s/%s Pi Heating Demand: %s" %(MsgClusterId, MsgSrcAddr, MsgSrcEp, MsgClusterData), MsgSrcAddr)
        value = ( value * 100 ) / 255
        checkAndStoreAttributeValue( self, MsgSrcAddr, MsgSrcEp,MsgClusterId, MsgAttrID,  MsgClusterData )
        ...
        if eurotronics:
            ...
            if MsgAttrID == '4000': # TRV Mode for EUROTRONICS
                ...
                self.log.logging( "Cluster", 'Log', "ReadCluster - 0201 - %s/%s Valve position: %s" %(MsgSrcAddr, MsgSrcEp,value), MsgSrcAddr)
                checkAndStoreAttributeValue( self, MsgSrcAddr, MsgSrcEp,MsgClusterId, MsgAttrID,  value )
...
def Cluster0702( self, Devices, MsgSQN, MsgSrcAddr, MsgSrcEp, MsgClusterId, MsgAttrID, MsgAttType, MsgAttSize, MsgClusterData , Source):
    ...
    if MsgAttrID == "0000": # CurrentSummationDelivered
        ...
        if MsgAttrID in ( '2000', '2001', '2002'): # Lx phase Power
            ...
            self.log.logging( "Cluster", 'Debug', "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Power %s" %( MsgClusterId, MsgSrcAddr, fake_ep, line, conso))
            ...
            self.log.logging( "Cluster", 'Debug', "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Voltage %s" %( MsgClusterId, MsgSrcAddr, fake_ep, line, value))
            ...
            self.log.logging( "Cluster", 'Debug', "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Current %s" %( MsgClusterId, MsgSrcAddr, fake_ep, line, value))
            ...
            self.log.logging( "Cluster", 'Debug', "readCluster - %s - %s/%s CASAIA PC321 phase Power Line: %s Summation Power %s" %( MsgClusterId, MsgSrcAddr, fake_ep, line, conso))
