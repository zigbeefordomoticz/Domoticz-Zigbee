...
def _domo_maj_one_cluster_type_entry( self, Devices, NwkId, Ep, device_id_ieee, model_name, ClusterType, ClusterTypeList, ClusterId, value, Attribute_, Color_, WidgetEp, Widget_Idx, WidgetType ):
    ...
    if "WindowCovering" in ClusterType and WidgetType in ("VenetianInverted", "Venetian", "Vanne", "VanneInverted", "WindowCovering", "Curtain", "CurtainInverted", "Blind") and Attribute_ != "f001":
...
def baro_adjustement_value(self, Devices, NwkId, DeviceId, Device_Unit):
    if self.domoticzdb_DeviceStatus:
        try:
            return round(self.domoticzdb_DeviceStatus.retrieve_baro_adjustment(domo_read_Device_Idx(self, Devices, DeviceId, Device_Unit,)), 1)
...
def temp_adjustement_value(self, Devices, NwkId, DeviceId, Device_Unit):
    if self.domoticzdb_DeviceStatus:
        try:
            return round(self.domoticzdb_DeviceStatus.retrieve_temp_adjustment(domo_read_Device_Idx(self, Devices, DeviceId, Device_Unit,)), 1)
        except Exception as e:
