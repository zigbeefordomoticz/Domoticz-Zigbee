...
def receive_poll_cluster(self, srcnwkid, srcep, cluster, dstnwkid, dstep, Sqn, ManufacturerCode, Command, Data):
    ...
    self.log.logging( "PollControl", "Debug", "receive_poll_cluster %s/%s %s %s %s" % (srcnwkid, srcep, cluster, Command, Data))
...
def poll_checkin_command(self, Sqn, snwkid, ep):
    ...
    self.log.logging( "PollControl", "Debug", "poll_checkin_command %s %s" % (snwkid, ep))
    cmd = "00"
    if snwkid not in self.ListOfDevices:
        self.log.logging( "PollControl", "Error"," - nwkid: %s do not exist" % snwkid)
        return
    ...
    self.log.logging( "PollControl", "Debug", "Accept Fast Poll command 0x%s for %s/%s with payload: %s" % (cmd, snwkid, ep, payload))
    return
...
def poll_checkin_response_command(self, Sqn, nwkid, ep, ContinueFastPoll=True, DurationFastPoll=0xFC0):
    ...
    self.log.logging( "PollControl", "Debug", "poll_checkin_response_command %s/%s %s %s" % (nwkid, ep, ContinueFastPoll, DurationFastPoll))

    cmd = "00"  # Check-in Response
    if nwkid not in self.ListOfDevices:
        self.log.logging( "PollControl", "Error", " - nwkid: %s do not exist" % nwkid)
        return
    cluster_id = "0020"  # Poll Control Cluster
    ...
    self.log.logging( "PollControl", "Debug", "Accept Fast Poll command 0x%s for %s/%s with payload: %s" % (cmd, nwkid, ep, payload))


def poll_fast_poll_stop(self, Sqn, nwkid, ep):

    # Fast Poll Stop to be called for Remote Devices
    self.log.logging( "PollControl", "Debug", "poll_fast_poll_stop %s/%s" % (nwkid, ep))
    cmd = "01"  # Fast Poll Stop ( no data)

    if nwkid not in self.ListOfDevices:
        self.log.logging( "PollControl", "Error", "Fast Poll Stop - nwkid: %s do not exist" % nwkid)
        return
    ...
    payload = cluster_frame + Sqn + cmd
    raw_APS_request(self, nwkid, ep, "0020", "0104", payload)
    self.log.logging( "PollControl", "Debug","send Fast Poll Stop command 0x%s for %s/%s with payload: %s" % (cmd, nwkid, ep, payload))


def poll_set_long_poll_interval(self, Sqn, nwkid, ep, NewLongPollInterval=0x14):

    self.log.logging( "PollControl", "Debug", "poll_set_long_poll_interval %s/%s %s" % (nwkid, ep, NewLongPollInterval))
    cmd = "02"
...
def poll_set_short_poll_interval(self, Sqn, nwkid, ep, NewShortPollInterval=0x2):
    ...
    self.log.logging( "PollControl", "Debug","poll_set_short_poll_interval %s/%s %s" % (nwkid, ep, NewShortPollInterval))
    cmd = "03"
